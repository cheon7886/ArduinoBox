/*
  ...
  History:
	...
	21/jan/14 - added support for ATmega16(Unofficial user)
...
void MsTimer2::set(unsigned long ms, void (*f)()) {
	...
	TIMSK &= ~(1<<TOIE2);                                    // 일단 오버플로우 인터럽트 비트 클리어(인터럽트 Off) 
	TCCR2 &= ~((1<<WGM21) | (1<<WGM20));                     // Normal 모드
	ASSR &= ~(1<<AS2);                                       // Timer/Counter2에만 있는 옵션 비트 off 
	TIMSK &= ~(1<<OCIE2);                                    // 컴페어 매치 인터럽트 비트 클리어(이 라이브러리에서 이 인터럽트가 사용되는 일은 없음)
...
void MsTimer2::start() {
	...
	TCNT2 = tcnt2;
	TIMSK2 |= (1<<TOIE2);
#elif defined (__AVR_ATmega128__)||defined (__AVR_ATmega16__)
	TCNT2 = tcnt2;
	TIMSK |= (1<<TOIE2);
#elif defined (__AVR_ATmega8__)
	TCNT2 = tcnt2;
...
#if defined (__AVR_ATmega168__) || defined (__AVR_ATmega48__) || defined (__AVR_ATmega88__) || defined (__AVR_ATmega328P__) || (__AVR_ATmega1280__)
	TIMSK2 &= ~(1<<TOIE2);
#elif defined (__AVR_ATmega128__)||(__AVR_ATmega16__)
	TIMSK &= ~(1<<TOIE2);
#elif defined (__AVR_ATmega8__)
	TIMSK &= ~(1<<TOIE2);
#endif
