...
#define START_SW_ON         ((~PIND) & 0b01000000)   // ONLY Hunoi
...
//	}
	...
	if(bootuart == 1) {
     ...
     UCSR0A = 0x0;           
     UCSR0B = 0b00011000;     // 송수신 모두 가능하게 함, 수신완료 인터럽트 disable
     UCSR0C = 0b10000110;     // 비동기 데이타 8bit 모드 
     UBRR0H = 0;              // 비동기 일반모드 57600bps
     UBRR0L = 16;
	}
	if(bootuart == 2) {
	...
	//스마트 보드 다운로드 포함
     DDRD |= 0b11000000;             // Hunoi보드 시리얼 통신 Enable 단자 on(PD7)
     PORTD |= 0b11000000;
